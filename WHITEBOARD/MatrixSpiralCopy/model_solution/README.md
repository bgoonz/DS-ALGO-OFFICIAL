...
doesn't actually work for our simple example. This function returns `[1, 2, 2, 4]`, not the expected answer of `[1, 2, 4, 3]`. Why is this the case?
...
Our while loop keeps running so long as `top_row <= bottom_row` and `left_column <= right_column`, but these variables are being updated within the body of the
...
the height of the matrix _ the width. Hence the running time of our algorithm is
O(n _ m).
